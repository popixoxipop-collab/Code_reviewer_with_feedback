...
**문제 4 — "관용 패턴 확정"이 너무 쉽게 이뤄진다. (→ Reflection에 한해 해소, D32~D34)**
...
**후속 조치(D34)**: "OR-3(4개 중 아무 3개)"으로 단순 완화하기 전에 먼저 실측 프로브를
넣어봤다 — "그래서 백엔드에서 제한해야 합니다."(자기오류인식 없이 이유·새판단·개선안
3개만 있는 문장)를 넣었더니 optional_matches=3으로 **순수 OR-3 기준을 그대로 통과해버렸다.**
이건 "애초에 틀린 적 없이 처음부터 맞았던 답변"까지 reflection으로 오판하게 만드는
치명적 결함이라, 순수 OR-3은 채택하지 않고 **"self_error_recognition은 항상 필수 + 나머지
3개 중 2개 이상"**으로 재설계했다. 3건 재검증 결과: B안 모범 예시=True, 피상적 답변=False,
자기오류인식 없는 프로브=False — 전부 의도대로 나왔다. **핵심 교훈: "몇 개 중 몇 개"라는
완화는 어떤 서브신호를 완화 대상에서 빼야 하는지부터 먼저 검증해야 한다.**
