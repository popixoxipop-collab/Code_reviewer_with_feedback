...
| R5 | 학생 신체정보(이름/키/나이/활동회원여부) 입력받아 저장+키 평균/최고/최저 통계+이름 포맷팅 명단 출력 | 02 변수와자료형 (p.30,36,38...) | StudentProfile(모델) · ProfileInputHandler(입력) · HeightAnalyzer(키 통계) · **ProfileReport(명단 포맷팅 출력, InputHandler가 사용)** · **HeightSummary(통계 등급 판정, Analyzer가 사용)** · Main |
| R6 | 장바구니 상품 입력+수량·가격 연산으로 소계/부가세/할인 계산+영수증 출력+상품 수 요약 | 04 Java의 연산자 재사용(다른 과제) | CartItem(모델) · CartInputHandler(입력) · PriceCalculator(연산) · **ReceiptPrinter(영수증 출력, Calculator가 사용)** · **CartSummary(상품수 요약, InputHandler가 사용)** · Main |
| R7 | 대출 신청자 신용점수 조건별 금리 등급 판정+상환방식 입력+월이자 계산+상환일정표 생성 | 05 조건문 재사용(다른 과제) | LoanApplication(모델) · InterestRateEvaluator(조건판정) · RepaymentInputHandler(입력) · **InterestReport(월이자 계산 출력, RateEvaluator가 사용)** · **RepaymentSchedule(일정표 생성, InputHandler가 사용)** · Main |
| R8 | 학생별 출석 반복 입력+출석률 반복 계산+기준 미달자 경고 탐색+전체 명단 반복 출력 | 06 반복문 재사용(다른 과제) | AttendanceRecord(모델) · AttendanceInputHandler(입력) · AttendanceRateCalculator(반복계산) · **LowAttendanceAlert(미달자 탐색 경고, RateCalculator가 사용)** · **AttendanceSummaryReport(명단 반복 출력, InputHandler가 사용)** · Main |
| R9 | 센서 측정값(ID/온도 double/습도 int) 입력+자료형 범위 검증(형변환 포함)+이상치 판정+전체 로그 출력 | 02 변수와자료형 재사용(다른 과제) | SensorReading(모델) · ReadingInputHandler(입력) · ReadingValidator(자료형 범위검증) · **AnomalyReport(이상치 판정 출력, Validator가 사용)** · **ReadingLog(전체 로그 출력, InputHandler가 사용)** · Main |
| R10 | 도서 대출/반납 정보 입력+연체 여부 판정+반납 이력 출력 | 03 사용자로부터입력받기 재사용(다른 과제) | BookLoan(모델) · LoanInputHandler(대출 입력) · ReturnInputHandler(반납 입력) · **OverdueChecker(연체 판정, LoanInputHandler가 사용)** · **LoanHistoryReport(반납 이력 출력, ReturnInputHandler가 사용)** · Main |

### R5~R10 확장 근거 (D134, 사용자 지시: "R100까지 늘려서 보여줘" → AskUserQuestion으로 "작은 규모로 실제 확장" 확정)

unit_map(`benchmarks/_curriculum_fixed_unit_map_cache.json`) 확인 결과 Java 커리큘럼은 정확히 6유닛(01~06)뿐. R1~R4가 이미 03~06을 다 썼고, **Unit01("Java 시작하기")은 87개 개념 중 상당수가 JVM/JDK/WORA/IDE 사용법 같은 비-코딩 배경지식이거나 이미 Unit02와 겹치는 기초 변수/출력 개념이라, "개념 범위가 유닛 하나로 국한"이라는 형태 등가성 기준을 지킬 만한 독자적 코딩 과제 소재가 없음** — 억지로 끼워맞추지 않고 정직하게 제외. Unit02(변수와자료형, 35개 개념)만 진짜 새로운 유닛. 나머지(R6~R10)는 이미 쓴 유닛을 **완전히 다른 과제 테마**로 재사용 — 위 "과제 등가성" 기준이 요구하는 건 "회차마다 다른 유닛"이 아니라 "과제 하나가 유닛 하나로 국한"이므로, 재사용 자체는 방법론 위반이 아님(같은 유닛 재사용 시 테마·모델·의존관계 전부 이전 회차와 겹치지 않게 신규 설계). 진짜 병목은 코퍼스 고갈이 아니라 실행 시간(회차당 코딩 에이전트+P02+인터뷰 2종+Hook File 2종+감사가 전부 순차 체이닝) — R10에서 정직하게 멈춤.
