...
- **D187** ([`docs/lab/trainee/session.html`](./docs/lab/trainee/session.html)) — 사용자가 세션 화면 스크린샷과 함께 요청: "검증 세션 시작 버튼 눌렀을 때 모델을 선택한 뒤에 시작되도록 로직 바꾸고 싶음."
  - **문제**: D178("한 번 클릭으로 자동실행")을 "P02 결과 화면 클릭 → session.html 진입 즉시 자동 실행"으로 구현했는데, 이 즉시-자동실행이 D182에서 추가한 모델 선택기의 존재 의의를 사실상 지워버림 — `P03Engine.run()`이 `hooks.onRunStart()`에서 모델을 잠그는데, 그 시점이 페이지 로드 직후라 실제로 모델을 바꿔볼 시간이 사실상 없었음(코드 패널 렌더링되는 찰나뿐).
  - **적용**: `.chatbody` 안에 모델 선택 게이트(`#start-gate` + `#start-btn`)를 추가 — `P03Engine.run()` 호출을 `DOMContentLoaded` 핸들러에서 분리해 `#start-btn` 클릭 핸들러로 이동. 게이트가 떠 있는 동안 모델 칩은 그대로 클릭 가능(비활성화 안 함), 클릭 후에야 `hooks.onRunStart()`가 정상적으로 `.modelbar`를 잠금. D178이 없앤 "굳이 필요없는 확인 클릭"과는 성격이 다름 — D178의 클릭은 "시작할지 말지"(선택지 없음)였고, 이번 클릭은 "11종 중 어느 모델로"(실제 비용이 걸린 진짜 선택).
  - **검증**: Playwright로 실제 NVIDIA 키 사용 — (1) 페이지 로드 후 2초 대기, 프록시로 나가는 요청 0건 확인(자동실행 완전히 제거됨), (2) 게이트 뜬 상태에서 `.modelbar`에 `.locked` 클래스 없음(칩 클릭 가능) 확인, (3) `mistral-medium-3.5` 칩 클릭 → 실제로 활성 칩 전환 확인, (4) `#start-btn` 클릭 → 실제 L1 질문이 바뀐 모델로 생성됨(프록시 요청 19건 발생, Pyodide 분류기 로드 포함) + `.modelbar`에 `.locked` 클래스 정상 부여 확인. 스크린샷으로 게이트 UI 육안 확인.
  - WHY: 사용자가 스크린샷으로 실제 UX 문제(모델 고를 틈이 없음)를 직접 지적.
...
- **D193** ([`docs/lab/db.js`](./docs/lab/db.js), [`docs/lab/p03-engine.js`](./docs/lab/p03-engine.js), [`docs/lab/p03-runner.js`](./docs/lab/p03-runner.js)) — 사용자가 Supabase 대시보드 스크린샷으로 "내 이메일 이외 다른 팀원들도 시도 중인데 왜 안 보여?" 문의. 조사 결과(Management API 직접 조회) `p03_turns_view`는 완주(`status='done'`, `artifacts.kind='transcript'` 존재)한 런만 보여주는 설계이고, 실제로는 팀원 6명 중 본인만 P03을 완료했으며 나머지는 아직 P02 단계이거나(4명) 1명은 NVIDIA 401로 즉시 실패(0턴)한 상태임을 확인 — 뷰/RLS 버그 아님. 후속 질문: "끝까지 완료 안하면 Findings나 그전에 대답했던 내용들은 다 날아가는 구조?" → 코드 확인 결과 실제로 그런 구조임을 발견, 즉시 수정 요청.
  - **조사**: `p03-engine.js`의 턴 답변(`transcript` 배열)은 루프 도는 동안 메모리에만 쌓이고, DB 저장(`LabDB.saveRun`→`artifacts` insert)은 전체 루프+채점이 끝난 뒤 `maybeSaveRun()` 단 한 번만 호출됨. 도중에 에러나면(`catch`) `LabApp.saveFailedRun()`이 `artifacts: []`를 하드코딩해 넘겨(`app.js:475`) 몇 턴을 답했든 전부 버려짐. `stage_events` 테이블(스키마엔 있었으나 전 파이프라인 0행, Management API로 직접 확인)이 애초에 이런 턴 단위 기록용으로 준비돼 있었는데 미사용 상태였음. RLS 정책 확인: `runs`는 insert own+**update own**+read all, `artifacts`/`stage_events`는 insert own+read all만(update 없음) — "런은 일찍 만들고 나중에 update, 턴은 순수 append-only insert" 설계가 새 RLS 정책 추가 없이 그대로 맞아떨어짐.
  - **적용**: `db.js`에 `startRun()`(런 시작 시 즉시 `runs` insert, `status:'running'`)/`logTurn()`(턴마다 `stage_events` insert) 신설, `saveRun()`에 `run_id` 옵션 추가(있으면 UPDATE, 없으면 기존과 100% 동일한 INSERT — P01/P02 무영향). `p03-engine.js`/`p03-runner.js`의 `run()`: 루프 진입 전 `startRun()`으로 `dbRun` 확보 → 매 턴 답변 직후 `logTurn()` → 성공 시 같은 row를 `status:'done'`+전체 artifacts로 UPDATE, 실패 시(`catch`) `dbRun` 있으면 같은 row를 `status:'error'`로 UPDATE(턴 데이터는 안 건드림), `dbRun`이 없을 때만(DB 미설정 또는 `startRun` 자체 실패) 기존 `saveFailedRun` 폴백 유지. 가시성 확보용 신규 뷰 `p03_progress_view`(`p03_turns_view`와 동일 `p02_matches` CTE 재사용, `artifacts` 대신 `stage_events` 조인) 추가 — 기존 `p03_turns_view`는 무수정.
  - **구현 중 발견한 버그 2건**: (1) 코드 재검토 중 `dbRun`을 `try` 블록 안에서 `let`으로 선언해놓고 바깥 `catch`에서 참조하려던 스코프 실수를 직접 발견 — JS는 `try` 안의 `let`이 대응하는 `catch`에서 안 보임, 그대로 뒀으면 에러 발생 시(이 기능이 정확히 필요한 순간) `ReferenceError`로 죽었을 것. `try` 앞으로 선언 이동해 수정. (2) 같은 실수로 만든 신규 hook `task-transcript-guard.py`(아래 환경 이슈 참고)가 배경 Bash 태스크의 정상 `.output` 로그까지 오차단하는 걸 바로 다음 사용에서 발견 — 심볼릭 링크 여부로 재구분(subagent transcript는 심볼릭 링크, 일반 Bash 로그는 실 파일)해 즉시 수정.
  - **환경 이슈**: 구현 도중 로컬 `~/Desktop/Code_reviewer_with_feedback/`(및 `Team-IZ-AI/`) 전체가 macOS Full Disk Access 권한 문제로 읽기/쓰기 모두 막힘(사용자가 터미널 재시작 없이 진행 요청) — GitHub Contents/Git Data API(blob→tree→commit→ref 순서로 직접 구성)로 로컬 디스크를 거치지 않고 원격에서 직접 fetch+수정+커밋+push.
  - **검증**: (1) 3개 파일 전부 `node --check` 통과. (2) push 후 GitHub Pages 재배포를 폴링 확인, **실제 라이브 배포된 `p03-engine.js`를 fetch해 Node vm에 로드**(재구현 아님)하고 `LabApp`/`LabDB`/`LabConfig`/`LabLLM`/`LabPyodide` 경계만 모킹해 `run()`을 직접 실행 — D190이 쓴 것과 동일한 "실파일 로드+경계만 모킹" 방법론. 시나리오 A(정상 2턴 완주): `startRun` 1회, `logTurn` 정확히 2회(seq 0:l1, 1:l2), `saveRun`이 같은 `run_id`로 `status:'done'`+transcript 2건+grades로 UPDATE — 통과. 시나리오 B(turn 2에서 NVIDIA 401류 실패 주입, 이 수정의 핵심 대상): turn 1은 `logTurn`으로 정상 기록(1회), turn 2 실패 시 `saveRun`이 **같은 run_id**로 `status:'error'`+`artifacts:[]`로 UPDATE, 구 폴백 `saveFailedRun`은 **호출 안 됨**(0회) — 통과. 시나리오 C(DB 미설정): `startRun`/`logTurn`/`saveRun` 전부 0회 호출, "Supabase 미설정" 메시지 정상 표시 — 기존 동작과 100% 동일, 통과. (3) Management API로 **실제 프로덕션 스키마에 동일 SQL 시퀀스를 직접 실행**해 재확인: `runs` insert(`status:'running'`, `finished_at:null`) → `stage_events` insert(turn 1) → 실패 경로 `runs` UPDATE(`status:'error'`) → **turn 1의 `stage_events` row가 그 UPDATE 이후에도 그대로 살아있음을 `count(*)=1`로 직접 확인**(이 수정의 핵심 주장 그 자체) → `p03_progress_view`에서 그 턴이 정확히 조회됨(question/answer/verdict 포함) → `p03_turns_view`에서는 (의도대로) 제외됨(`count(*)=0`, 완주 안 했으므로) — 테스트 데이터는 검증 직후 삭제, 실제 팀 대시보드에 남지 않음.
  - WHY: P03가 사람이 직접 여러 턴 답하는 흐름이라 늦은 턴 실패 시 손실이 실제로 크다 — 이번 실사례는 0턴 실패라 무해했지만 구조적 위험은 실재했고, 다음에 3턴째까지 답한 팀원이 같은 상황을 겪으면 그 답변이 전부 사라진다.
  - COST: 런당 DB 왕복 1회(시작)+턴당 최대 2회(현재 max_turns 설정 기준) 추가 — 텍스트 저장이라 지연 무시 가능. `logTurn`/`startRun` 실패는 검증 흐름을 막지 않도록 방어했지만, 그만큼 "턴별 저장 안 됐는데 진행 중"인 채로 넘어갈 여지가 생김(progress 메시지로만 안내).
  - EXIT: `stage_events`를 다른 용도로 쓰게 되면 별도 테이블 분리, `p03_progress_view`는 `artifacts` 기반으로 후속 통합 가능. `task-transcript-guard.py`는 harness가 TaskOutput 자체에서 큰 콘텐츠를 자동 절단하게 되면 불필요.
  - `Team-IZ/AI` 저장소의 동일 파일(`shared/db.js`, `shared/p03-engine.js`)에도 같은 수정 적용(blob SHA 동일 확인 — byte-identical). `p03-runner.js` 대응 파일은 없음(기존 컨벤션대로 trainee 페이지 전용 미러 범위 밖).
  - 커밋: `8617087`(fix, Code_reviewer_with_feedback main), `88ae1e5`(Team-IZ/AI feature/verification-ui), 둘 다 push 완료(GitHub Pages 재배포로 라이브 반영 확인됨). `p03_progress_view`는 Supabase Management API로 직접 생성.

- **D194** ([`judgment/importance_rank.py`](./judgment/importance_rank.py), [`judgment/score_findings.py`](./judgment/score_findings.py), [`hookfile/generate_hook_file.py`](./hookfile/generate_hook_file.py)) — 사용자 질문("findings 추출하는 알고리즘이 뭐야")에 답하다가 발견: `priority` 문자열은 detector 종류별 고정 라벨일 뿐 증거 기반 값이 아니고, `subrubric`의 3축(design_intent/question_value/risk)은 서로 독립 계산돼 이걸 하나로 합치는 로직이 코드 어디에도 없음. 사용자 지적("현 findings는 코드 map의 의존 관계 추출만 기계적으로 생성 후 분석하는 느낌") → Fable(Plan) 에이전트에 코드 실독 기반 설계 위임, 나 자신이 5건(파일 존재/버킷-tie 83.7%·6·16종/hookfile 버그 실재/D6 arXiv:2603.18059 인용/idiom 상태파일 구조)을 독립 재계산·재현해 정확함 확인 후 그 설계(Phase 0)를 그대로 구현.
  - **진단(설계 문서 실측)**: 40개 example repo·104개 finding 코퍼스에서 버킷 3중조(design_intent,question_value,risk)는 단 6종뿐이고 87/104(83.7%)가 (중,상,하) 하나에 몰림 — 반면 원점수(subrubric.*.total, 0~12)는 16종으로 갈라짐(D28 EXIT가 이미 원점수 직접소비를 승인해둔 상태였음). `hookfile/generate_hook_file.py`가 `priority` 문자열로 이미 순서를 매기려 시도했었는데, `idiom_filter.py`가 강등 시 그 문자열에 `" → 관용 패턴 필터 적용됨..."`을 덧붙여 변형시켜서 매치 실패 → `.get(priority, 9)` 최하위 폴백으로 조용히 밀리는 실재 버그를 발견(`examples/study_match`에서 1건 실측 재현).
  - **적용**: 신규 `judgment/importance_rank.py::apply_rank()` — 3축 **원점수**(버킷 아님)를 가중합해 `rank`/`rank_score`/`rank_evidence`를 붙이고 그 순서로 재정렬. idiom 강등 finding은 `score_risk()`가 이미 쓰는 것과 동일한 게이팅 방식(감산 아니라 `THRESHOLDS["중"]-1` 상한 캡)으로 처리. 가중치(`RANK_WEIGHT_QV/RISK/DI`, 전부 1.0)는 `judgment/rank_weights/rank_weights.json`에서 읽고, 라벨링 스프린트(`benchmarks/judgment_precision_labels.jsonl`, 50건, 아직 라벨 미완료) 전이라 D181 `MAX_CONNECT_FILES` 주석 스타일로 "unmeasured-provisional" 명시. 동점 처리 체인: rank_score desc → risk_total desc → trigger_confidence desc → 파일 fan_in desc → id asc(재현성 보장). `score_findings.py`에 한 줄 통합(`apply_idiom_filter`+`_tag_lang` 직후 — idiom 강등 이후 상태를 봐야 게이팅이 맞게 걸림). `hookfile/generate_hook_file.py`는 `rank` 있으면 그걸로, 없으면(구 artifacts) 기존 `priority_rank` 딕셔너리로 폴백 — 위 버그를 부수효과로 고침. `docs/lab/prompt_manifest.json`에 `p02-5`(중요도 랭킹) 스테이지 추가해 웹 랩 파라미터 편집기에서 가중치 override 가능하게, `webtool_driver.py`/`p02-engine.js`·`p02-runner.js`의 `collectOverrides`에 배선. `docs/lab/trainee/submission.html`/`p02-runner.js`의 finding 카드에 `#순위 (점수점)` 배지 표시.
  - **구현 중 발견·수정한 버그**: 첫 구현에서 동점 처리 체인만 정렬키로 쓰고 **`rank_score` 자체를 정렬키에서 빠뜨림** — `examples/study_match` 실측 중 발견(idiom 강등돼 rank_score가 4개 중 최저였던 finding이 rank=2로 나옴, 최하위여야 정상). `_sort_key`에 `rank_score`를 최우선 항목으로 추가해 수정, 같은 코퍼스로 재실측해 정상화(rank=4로 정확히 밀림) 확인.
  - **Team-IZ/AI 미러 범위**: 설계 문서는 "P02 관련 파일이 Team-IZ/AI엔 없다"고 가정했으나, 실제로 `shared/p02-engine.js`/`webtool_driver.py`/`prompt_manifest.json`/`trainee/submission.html` 4개 전부 존재하고 메인 저장소 변경 전 버전과 byte-identical(diff로 직접 확인 — `submission.html`은 경로 접두어만 다름)한 살아있는 미러로 확인돼, 가정을 버리고 4개 전부 동일 패치 적용. `p02-runner.js`는 `Team-IZ/AI/reference/`(명시적 비활성 스크래치 사본)에만 있어 미러 대상에서 제외(기존 컨벤션 유지).
  - **범위 밖(설계 문서 Phase 1~3, 이번엔 미착수)**: S1~S5 근거 보강 로직(`judgment/rank_evidence.py`), `benchmarks/fit_rank_weights.py`(라벨 50건 미완료라 전제조건 미충족), `judgment/rank_hook.py`(재귀 피드백 — P03 verdict 데이터 아직 부족).
  - **검증**: (1) 신규/수정 `.py`·`.js`·`.json` 전부 `py_compile`/`node --check`/`json.tool` 통과. (2) **회귀 안전(핵심)**: `examples/` 대신 이 저장소 자신의 `judgment/`를 실제 repo_root로 `two_tier_scan.scan()`+`score_findings.score()`를 두 번(patch 전/후를 몽키패치로 시뮬레이션) 실행해 11개 finding 전부 필드 단위 diff — id 집합·hub·기존 필드 전부 byte-identical, 새 필드(`rank`/`rank_score`/`rank_evidence`) 3개만 추가됨을 프로그램적으로 확인(육안 대조 아님). (3) 재현성: 동일 입력 2회 실행 rank 순서 완전 동일(`score()` 전체 레벨에서 재확인). (4) `examples/javascript/chargebee-node`(설계 문서가 지목한 최악 케이스, tier-b 5-way 동점) 실측 — rank_score 5개 전부 10.667로 진짜 동점이지만 tie_break_depth=4(id까지 내려가 완전 결정)로 정상 분리. (5) `examples/study_match`(idiom 강등 finding 포함) 실측 — 위 버그 수정 후 강등 finding이 정확히 최하위(rank=4)로 밀림 확인, 동시에 `hookfile/generate_hook_file.py`의 변형된 priority 문자열 케이스가 이제 `rank`로 정상 정렬됨도 같이 확인.
  - WHY: 사용자가 실제 알고리즘 설명을 요청한 뒤 "정렬 가능하냐"고 되물어 갭을 스스로 발견 → "코드 map 의존관계 추출만 기계적"이라는 정확한 비판에 대해, 새 스캔 기법이 아니라 **이미 만들어놓고 버리던 정보**(subrubric 원점수, Tier A edges/fan_in)를 조합하는 것으로 응답 — Fable 설계를 5건 실측 재검증 후 그대로 실행.
...
- **D195** ([`cognition/two_tier_scan.py`](./cognition/two_tier_scan.py), [`judgment/score_findings.py`](./judgment/score_findings.py), [`judgment/idiom_filter.py`](./judgment/idiom_filter.py), [`docs/lab/prompt_manifest.json`](./docs/lab/prompt_manifest.json)) — 사용자가 프론트엔드 빌드 산출물(webpack/vite 번들)이 finding 스캔에 섞여 나오는 것을 발견. 기존 필터는 정확한 디렉터리명 10종+확장자 매칭뿐이라, 이름이 매번 달라지는 contenthash 번들(`app.a1b2c3d4.js`)이나 목록에 없는 빌드 디렉터리는 그대로 통과했음.
  - **조사**: 웹 리서치로 같은 종류의 갭이 지원 언어 전반에 있음을 확인 — Java(Maven `target/` 없음), Python(`.pytest_cache/`/`.mypy_cache/`/`.tox/`/`.eggs/`/`*.egg-info` 없음), JS/TS(프레임워크 빌드 디렉터리 다수 없음: `.next/`/`.nuxt/`/`.output/`/`.nitro/`/`.svelte-kit/`/`.turbo/`/`.parcel-cache/`/`coverage/`/`storybook-static/`), C/C++(`cmake-build-*`/`.vs` 없음), Swift(`DerivedData/` 없음, 실사용 빈도는 낮음). 웹 리서치와 별개로 세 번째 갭을 이번 세션에서 직접 발견: `judgment/idiom_filter.py::_find_file_content`가 자체 하드코딩 스킵셋(5종 — 기존 두 파일의 10종 목록과조차 불일치)을 따로 갖고 있어, score_findings가 스캔에서 뺀 산출물을 `.h` c/cpp 재판정(D15) 같은 내용 조회 경로가 도로 읽어버리는 비대칭이 있었음.
  - **적용**: (1) 정확명 `SKIP_DIRS` 10→26종, (2) 패턴 기반 디렉터리 매칭 신설 — `SKIP_DIR_PREFIXES`(`cmake-build-*`)/`SKIP_DIR_SUFFIXES`(`*.egg-info`), 판정은 각 파일의 `_is_skip_dir()`, (3) 스킵 디렉터리 밖에 흩어진 산출물용 파일명 정규식 `GENERATED_FILENAME_RE`(contenthash 번들·`.min.js`·`.d.ts`) 신설. repo 트리를 걷는 세 파일에 동일 적용(디렉터리 상수 3종은 세 파일 전부, 파일명 정규식은 소스 파일을 선별하는 `two_tier_scan`/`score_findings` 2곳 — `idiom_filter`는 이미 선별된 finding의 파일을 이름으로 찾을 뿐이라 해당 없음). `idiom_filter.py`는 `from score_findings import SKIP_DIRS`가 불가(score_findings.py:7이 idiom_filter를 import — 순환, 실측 양방향 ImportError)해 독립 중복으로 해결, 작은 상수는 중앙화 대신 의도적으로 중복하는 기존 컨벤션(D13 EXIT/D76)을 따름. 웹 랩 배선: `prompt_manifest.json` p02-1에 `SKIP_DIRS` 기본값 동기화 + `SKIP_DIR_PREFIXES`/`SKIP_DIR_SUFFIXES`(string_list, 편집 가능) + `GENERATED_FILENAME_RE`(regex_readonly, 표시 전용 — p02-2/p02-3 정규식들과 같은 v1 관례) 추가. JS/드라이버는 무수정 — `collectOverrides()`(p02-engine.js/p02-runner.js)가 stage params를 제네릭하게 순회하고, `webtool_driver.py::apply_overrides()`의 기존 tuple/set 캐스팅이 새 파라미터를 그대로 처리함을 재확인.
  - **검증**: (1) 세 파일 상수 일치를 육안 아닌 직접 import 프로그램 비교로 확인 — `SKIP_DIRS`(26종·중복 없음)/`SKIP_DIR_PREFIXES`/`SKIP_DIR_SUFFIXES` 세 파일 전부 동일, `GENERATED_FILENAME_RE.pattern` 두 파일 동일, manifest 새 기본값도 전부 일치(ALL MATCH). (2) 합성 트리 스모크 테스트 — 산출물 10종(`.next/`·`target/`·`cmake-build-debug/`·`*.egg-info/`·`DerivedData/`·`.pytest_cache/`·`storybook-static/`·`*.min.js`·contenthash 번들·`*.d.ts`)을 심은 트리에서 `find_src_files()`가 실소스 2개만 반환, `_find_file_content()`가 `.next/` 안의 파일을 더 이상 읽지 않음을 실행으로 확인. (3) manifest는 json 재직렬화 없이 텍스트로만 수정(json.dump가 ~1,500줄 전체를 재포맷하는 함정 회피) — `git diff --stat` 4 insertions/1 deletion, JSON 파싱·파라미터 열거 정상.
  - WHY: 사용자가 실제 스캔 결과에서 빌드 산출물 finding을 직접 발견한 게 트리거 — 산출물은 리뷰 대상 코드가 아니라서 finding 하나하나가 순수 노이즈고, trainee가 가짜 finding 검증에 시간을 쓰게 만든다. 새 탐지 기법 추가가 아니라 스캔 입구의 필터 갭을 막는 수정.
