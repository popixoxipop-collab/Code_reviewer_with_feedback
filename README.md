...
- **D43** ([`PRESENTATION_SCRIPT.md`](./PRESENTATION_SCRIPT.md)) — TEAM_POC_SUMMARY.md(보고서)와 별도로 실제 구술 발표문을 작성
  - WHY: 팀 Notion 문서(1인칭 구어체 "그래서 발표에서는 이렇게 말하는 것이 맞다" 섹션, 2007행 부근 "팀 발표에서 '우리가 다 우월하다'고 말하면 안 되고 증명된 것과 안 된 것을 구분해서 말해야 한다"는 팀 원칙)를 확인 — 보고서(표 위주)와 발표문(구어체, 청중 설득 흐름)은 등록해야 할 정보가 다름. 발표문은 3블록 각각의 성공/실패를 있는 그대로 말하는 구조로 작성해 팀 원칙을 그대로 따름
  - COST: 판단 블록만 "성공"으로 명시하고 인지·피드백은 한계를 그대로 노출 — 발표에서 프로젝트를 과장해서 팔 수 있는 여지를 스스로 줄임
  - EXIT: 청중/시간 제약에 따라 분량 조절 필요 시 3블록 표만 남기고 나머지 축약 가능. 실제 대화형 검증(D42 EXIT)이 이뤄지면 결론 문단을 갱신
- **D44** (`TEAM_POC_SUMMARY.md` 구조 보강) — 실행한 3명(김만서/박진용/손진원)의 공통 구조 요소 중 빠졌던 3가지를 추가
  ...
  - COST: 문서 길이가 늘어나 발표 자료로 쓰기엔 `PRESENTATION_SCRIPT.md`(압축본)를 따로 유지해야 함 — 두 문서를 동기화해서 관리하는 부담이 생김
  - EXIT: 두 문서가 벌어지면 `TEAM_POC_SUMMARY.md`를 단일 소스로 삼고 `PRESENTATION_SCRIPT.md`는 그 요약본이라는 관계를 명시적으로 문서 상단에 표기
